...
class TBTT:
    ...
    def do_template_tracking(self, goal):
        ...
        TEMPLATE_CORNERS_3D = np.array([[ 0.310559, 0.310559, 0.],
                                        ...
                                        [-0.1552795,  0.310559, 0.]]).flatten()
        
        # Get the reference image
        REFERENCE = cv2.imread("./camera/src/correspondences_2d3d/Images/reference_image.png")
        REFERENCE = cv2.cvtColor(REFERENCE, cv2.COLOR_BGR2GRAY)
        
        # Detect and match SIFT points between the template and reference images
        TEMPLATE_POINTS, REFERENCE_POINTS = sift.detect_and_match(
            TEMPLATE, REFERENCE, nb_points=200)
        
        # Find the homography between the template image and
        # the reference image, with RANSAC to deal with outliers
        Fstar, _ = cv2.findHomography(TEMPLATE_POINTS, REFERENCE_POINTS, cv2.RANSAC, 3.0)
        
        # Get the position of the template corners in the reference image
        TEMPLATE_WIDTH, TEMPLATE_HEIGH = np.shape(TEMPLATE)
        TEMPLATE_CORNERS_2D = np.array([[0, 0],
                                        [TEMPLATE_WIDTH, 0],
                                        [TEMPLATE_WIDTH, TEMPLATE_HEIGH],
                                        [0, TEMPLATE_HEIGH]])
        mu_star = apply_homography(TEMPLATE_CORNERS_2D, Fstar)
        ...
        X_grid_template, Y_grid_template = np.meshgrid(X, Y)
        
        # Get the intensity of the points on the grid and store them in a column array
        i_star = np.float32(TEMPLATE[X_grid_template, Y_grid_template].reshape(NB_POINTS_2D, 1))
        
        # Get the corresponding grid in the reference image
        X_grid_star, Y_grid_star = grid_homography(X_grid_template, Y_grid_template, Fstar)
        ...
        # Detect and match SIFT points between the reference and current images
        REFERENCE_POINTS, points = sift.detect_and_match(
            REFERENCE, image_gray, nb_points=200)
        
        # Find the homography between the reference image and
        # the current image, with RANSAC to deal with outliers
        F, _ = cv2.findHomography(REFERENCE_POINTS, points, cv2.RANSAC, 3.0)
        
        # Get the position of the template corners in the current image
        mu = apply_homography(mu_star, F)
        
        # Initialize the grid in the current image
        X_grid, Y_grid = grid_homography(X_grid_star, Y_grid_star, F)
        
        
        
        X_grid_flat = X_grid.ravel()
        Y_grid_flat = Y_grid.ravel()
        # X_grid_flat_template = X_grid_template.ravel()
        # Y_grid_flat_template = Y_grid_template.ravel()

        points = np.zeros((NB_POINTS_2D, 2))
        # points_template = np.zeros((NB_POINTS_2D, 2))

        for k in range(NB_POINTS_2D):
            points[k, 0] = X_grid_flat[k]
            points[k, 1] = Y_grid_flat[k]
            # points_template[k, 0] = X_grid_flat_template[k]
            # points_template[k, 1] = Y_grid_flat_template[k]
        
        # print(points_template[1, :])
        print(points[:3, :])
        print(np.float32(image_gray[X_grid, Y_grid].reshape(NB_POINTS_2D, 1)))
        # print(TEMPLATE[int(points_template[2, 1])][int(points_template[2, 0])])
        # print(image_gray[int(points[2, 1])][int(points[2, 0])])
        # dw.draw_points(image_gray, points)
        cv2.imshow("kdfjs", image_gray)
        # cv2.imshow("jkdfl", TEMPLATE)
        cv2.waitKey()
        ...
        while not self.template_tracking_server.is_preempt_requested():
            ...
            correspondences.coordinates_2d = mu.flatten()
            ...
            for l in range(NB_LEVEL):
                ...
                for it in range(NB_IT):
                    ...
                    # cv2.imshow("kdfjs", image_to_display)
                    # # cv2.imshow("jkdfl", TEMPLATE)
                    # cv2.waitKey()
