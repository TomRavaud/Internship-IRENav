...
# DMU_MAX = 3
DMU_MAX = np.linspace(20, 3, NB_LEVEL)
...
# dw.draw_points(REF, points)
# dw.draw_points(REF, mu0, color=(0, 255, 0))
# dw.draw_quadrilateral(REF, mu0[:,::-1])

# cv2.imwrite("reference_image.png", REF)
...
for l in range(NB_LEVEL):
    ...
    for t in range(NB_TRANSFORMATIONS):
        ...
        dmu = np.random.rand(4, 2)*2*DMU_MAX[l] - DMU_MAX[l]
        ...
        # dw.draw_points(image_to_display, points, color=(0, 255, 0))
        # # dw.draw_points(image_to_display, mu0 + dmu, color=(0, 255, 0))
        # dw.draw_quadrilateral(image_to_display, mu0[:,::-1])
        
        # # cv2.imwrite("small_deformation.png", image_to_display)
