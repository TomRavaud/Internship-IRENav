...
if __name__ == "__main__":
    # Initialize the node
    rospy.init_node("world_camera_transform_publisher")
    
    # Initialize a publisher to the world_camera_transform topic
    ...
    # it)
    pub_transform = rospy.Publisher("world_camera_transform", TransformStamped, queue_size=1,
                                    latch=True)
    ...
    while not rospy.is_shutdown():
        ...
        try:
            # Get the transform between the world frame and the camera frame
            transform = tfBuffer.lookup_transform(
                "camera1/world",
                "camera1/camera_frame_oriented", rospy.Time())
